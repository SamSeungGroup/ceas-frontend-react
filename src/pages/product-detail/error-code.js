...
// 1-5. '시간 표현'를 위한 모듈 추가
import moment from "moment";                                                                 // momment 모듈: '시간' 표현

// 1-6. '삼승 CEAS UI 기능 컴포넌트' 추가
...
const ProductDetail = () => {
    ...
    {
        product_id: 1,
        product_name: "뉴 플라즈마 P",
        product_price: "9800",
        product_positive: "70",
        product_description: "부스터 파워를 경험하라!",
        userName: "브로디",
        userId: "brodi123",
        product_createDate: "202320227",
    };
    ...
    useEffect(() => {
        ...
        try{
            ...
            const getProductDetail = async () => {
                await axios.get(`http://localhost:8080/products/${product_id}`); // axios.get 메소드: '서버 주소'로부터 '데이터' 수신 -> '상품 상세 정보 데이터' 수신    
            ...
            getProductDetail().then((response) => {
                setProductDetail(response); // '상품 상세 정보' 설정
                setIsLoaded(true);          // '상품 상세 정보 출력' 설정
            })
        }
    ...
    useEffect(() => {
        ...
        try{
            ...
            const getUser = async () => {
                await axios.get(`http://localhost:8080/users/${id}`); // axios.get 메소드: '서버 주소'로부터 '데이터' 수신 -> '내 정보 데이터' 수신
    ...
    const loadPayment =  () => {
        ...
        const purchaseData = {
            ...
            merchant_uid: "20220216",                                      // merchant_uid 필드: 결제 번호
            name: product_Detail.product_name,                             // name 필드: 결제 상품 이름
            amount: product_Detail.product_price,                          // amount 필드: 결제 상품 금액(필수 항목)
        ...
        <React.Fragment>
            {isLoaded && (
                <div className = "product-detail_wrapper">
                    {
                        // jwtUtils.isAuth(token) && //  userId === product_Detail.userId &&
                    ...
                    <div className = "product-detail_header">
                        <div className = "product-detail-product-name">{ product_Detail.product_name }</div>
                    </div>
                    ...
                    <div className = "product-detail_body">
                        ...
                        <div className = "product-detail-title-content">
                            ...
                            <div className = "product-detail_description">{ product_Detail.userName }</div>

                            <h2 className = "product-detail_description">상품 가격</h2>
                            <div className = "product-detail_description">{ product_Detail.product_price }원</div>

                            <h2 className = "product-detail_description">상품 설명</h2>
                            <div className = "product-detail_description">{ product_Detail.product_description }</div>
                        </div>
                    </div>
